/*
 * poisson_compare_fence_corrected.c
 *
 ...
 */
 ...
 static inline int idx(int i,int j,int nx,int ny){
     return i*(ny+2) + j;
 }
 
 /* Initialize boundary conditions: u(x,0)=sin(pi x), rest zero */
 void init_bc(double *u, int nx, int ny, double h) {
     int L = (nx+2)*(ny+2);
     for(int k=0; k<L; k++) u[k] = 0.0;
                 ...
                 double tol,int maxiter)
 {
     ...
     do {
         ...
         /* exchange columns */
         MPI_Isend(&u[idx(nx,1,nx,ny)], 1, MPI_DOUBLE, east, 2, comm, &reqs[4]);
         MPI_Irecv(&u[idx(0,1,nx,ny)],   1, MPI_DOUBLE, west, 2, comm, &reqs[5]);
         MPI_Isend(&u[idx(1,1,nx,ny)],   1, MPI_DOUBLE, west, 3, comm, &reqs[6]);
 ...
 /* RMA fence-based ghost exchange with one column type only */
                     ...
                     MPI_Datatype coltype)
 {
     ...
     /* rows with contiguous put */
     if(north != MPI_PROC_NULL) {
         MPI_Put(&u[idx(1,ny,nx,ny)], ny, MPI_DOUBLE,
                 north, idx(1,0,nx,ny), ny, MPI_DOUBLE, win);
     }
     if(south != MPI_PROC_NULL) {
         MPI_Put(&u[idx(1,1,nx,ny)], ny, MPI_DOUBLE,
                 south, idx(1,ny+1,nx,ny), ny, MPI_DOUBLE, win);
     }
     MPI_Win_fence(0, win);
 }
