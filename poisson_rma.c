 ...
 * Compares RMA versions against baseline to verify correctness.
 */
 ...
 // Baseline MPI_Send/MPI_Recv ghost exchange\ nvoid exchange_mp(double *u, int nx, int ny, MPI_Comm comm2d) {
     int coords[2], dims[2], nbrs[4];
     ...
     // pack sends
     for(int j=1; j<=ny; j++) s_up[j-1]   = access_u(u, 1, j, ny+2);
     for(int j=1; j<=ny; j++) s_down[j-1] = access_u(u, nx, j, ny+2);
     for(int i=1; i<=nx; i++) s_left[i-1] = access_u(u, i, 1, ny+2);
     for(int i=1; i<=nx; i++) s_right[i-1]= access_u(u, i, ny, ny+2);
     ...
     // unpack
     for(int j=1; j<=ny; j++) set_u(u, 0,    j, ny+2, r_up[j-1]);
     for(int j=1; j<=ny; j++) set_u(u, nx+1, j, ny+2, r_down[j-1]);
     for(int i=1; i<=nx; i++) set_u(u, i,    0, ny+2, r_left[i-1]);
     for(int i=1; i<=nx; i++) set_u(u, i, ny+1, ny+2, r_right[i-1]);
 ...
 void exchange_fence(double *u, double *winbuf, int nx, int ny,
     ...
     MPI_Win_fence(0, win);
     // put halo rows/cols into neighbor windows
     for(int j=1; j<=ny; j++) {
     ...
     // copy from winbuf into u
     for(int j=1; j<=ny; j++) set_u(u, 0,    j, ny+2, winbuf[0*(ny+2)+j]);
     for(int j=1; j<=ny; j++) set_u(u, nx+1, j, ny+2, winbuf[(nx+1)*(ny+2)+j]);
     for(int i=1; i<=nx; i++) set_u(u, i,    0, ny+2, winbuf[i*(ny+2)+0]);
     for(int i=1; i<=nx; i++) set_u(u, i, ny+1, ny+2, winbuf[i*(ny+2)+ny+1]);
 }
 ...
 int main(int argc, char **argv) {
     ...
     // initialize
     for(int i=0; i<total; i++) winbuf[i] = 0.0;
     for(int i=0; i<total; i++) u_mp[i] = u_fence[i] = u_active[i] = 0.0;
     ...
     for(int i=0; i<total; i++) {
         double diff_f = fabs(u_mp[i] - u_fence[i]);
         double diff_a = fabs(u_mp[i] - u_active[i]);
         max_err_f = fmax(max_err_f, diff_f);
         max_err_a = fmax(max_err_a, diff_a);
     }
